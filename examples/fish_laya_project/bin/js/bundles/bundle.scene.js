...
(() => {
  ...
  var CameraCaptureEnv = class {
    ...
    static captureToFile() {
      return __async(this, null, function* () {
        ...
        try {
          ...
          yield rt.getDataAsync(0, 0, width, height, pixels);
          const filePath = yield CameraCaptureEnv._savePng(pixels, width, height, "scene");
    ...
    static captureFromSelectedCamera() {
      return __async(this, null, function* () {
    ...
    static captureSelectedCameraFromCommand() {
      return __async(this, null, function* () {
        ...
        try {
          ...
          const result = yield CameraCaptureEnv._captureCameraToFile({
    ...
    static _captureCameraToFile(options) {
      return __async(this, null, function* () {
        var _a;
        let tempRT = null;
        try {
          ...
          const sceneRT = (_a = EditorEnv.d3Manager) == null ? void 0 : _a.sceneRT;
          const width = (sceneRT == null ? void 0 : sceneRT.width) | 0 || 1280;
          const height = (sceneRT == null ? void 0 : sceneRT.height) | 0 || 720;
          ...
          const target = options.targetName ? CameraCaptureEnv._findNodeByName(scene3D, options.targetName) : null;
          tempRT = Laya.RenderTexture.createFromPool(
            ...
            1,
            false,
            options.renderTextureSrgb !== false
          );
          ...
          try {
            ...
            yield CameraCaptureEnv._renderCameraToTexture(camera, scene3D, tempRT, options.renderBackend || "draw_scene");
          } finally {
          ...
          const pixels = new Uint8Array(width * height * 4);
          yield tempRT.getDataAsync(0, 0, width, height, pixels);
          if (options.captureDebugMode === "color_only") {
            ...
            const maskPixels = yield CameraCaptureEnv._renderSilhouetteMask(camera, scene3D, tempRT, target, width, height, target.transform.localRotationEuler.clone());
            CameraCaptureEnv._applyMaskAlpha(pixels, maskPixels, options.maskAlphaMode, options.maskAlphaThreshold);
          }
          const filePath = options.outputDir && options.fileName ? yield CameraCaptureEnv._savePngToDir(pixels, width, height, options.outputDir, options.fileName, options.captureDebugMode !== "color_only" && options.zeroTransparentRgb !== false) : yield CameraCaptureEnv._savePng(pixels, width, height, options.prefix || "pose");
          console.log(`[CameraCaptureEnv] saved -> ${filePath}`);
    ...
    static captureMultiviewFromCommand() {
      return __async(this, null, function* () {
        var _a;
        ...
        try {
          ...
          const target = command.target_name ? CameraCaptureEnv._findNodeByName(scene3D, command.target_name) : null;
          ...
          const sceneRT = (_a = EditorEnv.d3Manager) == null ? void 0 : _a.sceneRT;
          const width = Math.max(1, command.width | 0 || (sceneRT == null ? void 0 : sceneRT.width) | 0 || 1280);
          const height = Math.max(1, command.height | 0 || (sceneRT == null ? void 0 : sceneRT.height) | 0 || 720);
          const views = command.views && command.views.length > 0 ? command.views : [{ view_id: "v000_yaw0_pitch0", yaw: 0, pitch: 0, file_name: "laya_v000_yaw0_pitch0.png" }];
          const outputDir = CameraCaptureEnv._resolveOutputDir(command.output_dir);
          if (!fs.existsSync(outputDir)) {
            fs.mkdirSync(outputDir, { recursive: true });
          }
          ...
          try {
            for (let index = 0; index < views.length; index++) {
              ...
              yield CameraCaptureEnv._settleEditorRenderState(refreshDelayMs);
              ...
              const renderBackendUsed = yield CameraCaptureEnv._renderCameraToTexture(camera, scene3D, tempRT, command.render_backend || "draw_scene");
              const pixels = new Uint8Array(width * height * 4);
              yield tempRT.getDataAsync(0, 0, width, height, pixels);
              ...
              if (debugColorOnly) {
                ...
                const maskPixels = yield CameraCaptureEnv._renderSilhouetteMask(camera, scene3D, tempRT, target, width, height, lockedLocalEuler);
              ...
              const filePath = yield CameraCaptureEnv._savePngToDir(pixels, width, height, outputDir, fileName, !debugColorOnly && command.zero_transparent_rgb !== false);
              saved.push(filePath);
              viewReports.push({
                index,
                view_id: viewId,
            ...
            yield CameraCaptureEnv._waitFrames(1);
          }
    ...
    static _pollAutoCapture() {
      return __async(this, null, function* () {
        ...
        try {
          ...
          const result = yield CameraCaptureEnv.captureMultiviewFromCommand();
    ...
    static _savePng(pixels, width, height, prefix) {
      return __async(this, null, function* () {
        const fs = IEditorEnv.require("fs");
        const pathMod = IEditorEnv.require("path");
        ...
        const fileName = `${prefix}_${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}.png`;
        ...
        yield sharp(pixelBuffer, {
          raw: { width, height, channels: 4 }
        }).png().toFile(filePath);
        return filePath;
      });
    }
    static _savePngToDir(pixels, width, height, outputDir, fileName, zeroTransparentRgb) {
      return __async(this, null, function* () {
        const pathMod = IEditorEnv.require("path");
        const sharp = IEditorEnv.require("sharp");
    ...
    static _zeroTransparentRgb(pixels) {
    ...
    static _renderSilhouetteMask(camera, scene3D, renderTexture, target, width, height, lockedLocalEuler) {
      return __async(this, null, function* () {
        const previousClearColor = camera.clearColor ? camera.clearColor.clone() : null;
        ...
        try {
          ...
          camera.clearColor = new Laya.Color(0, 0, 0, 1);
          yield CameraCaptureEnv._renderCameraToTexture(camera, scene3D, renderTexture, CameraCaptureEnv._currentRenderBackend);
          const maskPixels = new Uint8Array(width * height * 4);
          yield renderTexture.getDataAsync(0, 0, width, height, maskPixels);
          return maskPixels;
        } finally {
    ...
    static _renderCameraToTexture(camera, scene3D, renderTexture, backend) {
      return __async(this, null, function* () {
        ...
        if (backend === "camera_render_target") {
          ...
          try {
            ...
            yield CameraCaptureEnv._waitFrames(2);
            if (d3 && typeof d3.refresh === "function") {
              d3.refresh();
            }
            yield CameraCaptureEnv._waitFrames(1);
    ...
    static _restoreRenderState(states) {
    ...
    static _collectRenderSources(root) {
      const sources = [];
      CameraCaptureEnv._walk(root, (node) => {
    ...
    static _getSourceMaterials(source) {
      if (source.sharedMaterials && source.sharedMaterials.length !== void 0) {
        return Array.prototype.slice.call(source.sharedMaterials);
      }
      if (source._materials && source._materials.length !== void 0) {
    ...
    static _setSourceMaterials(source, materials) {
    ...
    static _applyMaskAlpha(pixels, maskPixels, mode, threshold) {
    ...
    static _liftRgbIntoAlpha(pixels, threshold) {
      const minValue = typeof threshold === "number" ? Math.max(0, Math.min(255, threshold)) : 1;
      for (let i = 0; i < pixels.length; i += 4) {
        if (pixels[i + 3] !== 0) {
          continue;
    ...
    static _collectRenderDiagnostics(scene3D, camera) {
      ...
      const queueLength = directionQueue && typeof directionQueue._length === "number" ? directionQueue._length : Array.isArray(queueElements) ? queueElements.length : 0;
      const queueLights = [];
      ...
      return {
        ...
        stat_enable_light: Laya.Stat ? Laya.Stat.enableLight : null,
        config3d_multi_lighting: Laya.Config3D ? Laya.Config3D._multiLighting : null,
    ...
    static _collectSceneDirectionLightNodes(root, scene3D) {
      const result = [];
      const walk = /* @__PURE__ */ __name((node) => {
        if (!node) {
          return;
        }
        const componentType = Laya.DirectionLightCom;
    ...
    static _resolveOutputDir(outputDir) {
      const pathMod = IEditorEnv.require("path");
    ...
    static _findNodeByName(root, name) {
      if (!root || !name) {
        return null;
      }
      if (root.name === name) {
    ...
    static _resolveCaptureMode(command, camera, target) {
    ...
    static _isDescendantOf(node, ancestor) {
    ...
    static _resolveCenter(command, target) {
    ...
    static _resolveRadius(command, target) {
    ...
    static _tryGetBounds(target) {
      let result = null;
      CameraCaptureEnv._walk(target, (node) => {
    ...
    static _placeCamera(camera, center, radius, view, command) {
    ...
    static _rotateTargetForView(target, baseEuler, view, command) {
    ...
    static _alignChildTargetToCamera(target, command) {
    ...
    static _applyMaterialPatch(command, fallbackTarget) {
      const patch = command.material_patch;
      if (!patch || !patch.values) {
        return { applied: false, materialCount: 0, valueCount: 0 };
      }
      try {
        const target = patch.target_name ? CameraCaptureEnv._findNodeByName(EditorEnv.scene.scene3D, patch.target_name) : fallbackTarget;
        if (!target) {
        ...
        CameraCaptureEnv._walk(target, (node) => {
          const renderer = node.meshRenderer || node.skinnedMeshRenderer || node.renderer;
          const sharedMaterials = renderer && renderer.sharedMaterials ? renderer.sharedMaterials : null;
          if (!sharedMaterials) {
        ...
        return { applied: false, materialCount: 0, valueCount: 0, error: e.message || String(e) };
      }
    }
    static _setMaterialValue(material, name, value) {
    ...
    static _refreshEditorScene(delayMs) {
      return __async(this, null, function* () {
        const d3 = EditorEnv.d3Manager;
        if (d3) {
          d3.refresh();
    ...
    static _settleEditorRenderState(delayMs) {
      return __async(this, null, function* () {
        if (delayMs > 0) {
          yield new Promise((resolve) => setTimeout(resolve, delayMs));
        }
    ...
    static _waitFrames(count) {
      return __async(this, null, function* () {
        for (let i = 0; i < count; i++) {
          yield new Promise((resolve) => Laya.timer.frameOnce(1, CameraCaptureEnv, resolve));
        }
      });
    }
    static _vec3ToArray(value) {
      return [value.x, value.y, value.z];
    }
    static _safeFileName(fileName) {
      return fileName.replace(/[\\/:*?"<>|]/g, "_").slice(0, 160) || "capture.png";
    }
    static _pad(value, width) {
  ...
  __name(CameraCaptureEnv, "CameraCaptureEnv");
  CameraCaptureEnv.COMMAND_FILE = "material_fit_capture_command.json";
  CameraCaptureEnv._autoTimer = null;
  CameraCaptureEnv._autoBusy = false;
  CameraCaptureEnv._lastAutoNonce = "";
  CameraCaptureEnv._currentRenderBackend = "draw_scene";
  __decorateClass([
    IEditorEnv.onLoad
  ], CameraCaptureEnv, "onLoad", 1);
  CameraCaptureEnv = __decorateClass([
    IEditorEnv.regClass("07234a45-2ad7-455a-b77a-ee5d07729416", "Editor/CameraCaptureEnv.ts")
  ], CameraCaptureEnv);
