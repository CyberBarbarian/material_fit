(function (exports, Laya) {
    ...
    class AnimationTemplet extends Laya.Resource {
        ...
        static _BezierInterpolation_6(bone, index, out, outOfs, data, dt, dData, duration, nextData, interData = null, offset = 0) {
            out[outOfs] = data[index] + (nextData[index] - data[index]) * Laya.Bezier.getRate(dt / duration, interData[offset], interData[offset + 1], interData[offset + 2], interData[offset + 3]);
            return 5;
        }
        static _BezierInterpolation_7(bone, index, out, outOfs, data, dt, dData, duration, nextData, interData = null, offset = 0) {
            out[outOfs] = interData[offset + 4] + interData[offset + 5] * Laya.Bezier.getRate((dt * 0.001 + interData[offset + 6]) / interData[offset + 7], interData[offset], interData[offset + 1], interData[offset + 2], interData[offset + 3]);
            return 9;
        }
        constructor() {
            super();
    ...
    class IkConstraint {
        ...
        _applyIk3(parent, child, targetX, targetY, bendDir, alpha) {
            ...
            _tempMatrix$1.identity();
            _tempMatrix$1.rotate(pRotation);
            _tempMatrix$1.scale(parent.resultMatrix.getScaleX(), parent.resultMatrix.getScaleY());
            _tempMatrix$1.translate(parent.resultMatrix.tx, parent.resultMatrix.ty);
            _tempMatrix$1.copyTo(parent.resultMatrix);
            parent.updateChild();
            var childRotation;
            childRotation = Math.atan2(targetY - cwy, targetX - cwx);
            child.setRotation(childRotation);
            var childTarMatrix;
            childTarMatrix = _tempMatrix$1;
            ...
            _tempMatrix$1.copyTo(child.resultMatrix);
            child.updateChild();
        }
    }
    IkConstraint.radDeg = 180 / Math.PI;
    IkConstraint.degRad = Math.PI / 180;
    const _tempMatrix$1 = new Laya.Matrix();
    ...
    class Skeleton extends Laya.Sprite {
        ...
        set source(value) {
            this._source = value;
            if (value) {
                Laya.ILaya.loader.load(value).then((templet) => {
        ...
        load(path, complete) {
            Laya.ILaya.loader.load(path).then((templet) => {
                ...
                this.init(templet);
                this.play(0, true);
                if (complete instanceof Laya.Handler)
    ...
    class BoneSlot {
        ...
        getSaveVerticle(tArr) {
            if (BoneSlot.useSameMatrixAndVerticle && this._preGraphicVerticle && BoneSlot.isSameArr(tArr, this._preGraphicVerticle)) {
                ...
                tArr = Laya.Utils.copyArray([], tArr);
                        ...
                        if (this._parentMatrix) {
                            ...
                            if (tCurrentMatrix) {
                                ...
                                if (this._diyTexture && this.currDisplayData.uvs) {
                                    ...
                                    Laya.Matrix.mul(_tempMatrix, Laya.Matrix.TEMP, tResultMatrix);
                                }
                                else {
    ...
    const _tempMatrix = new Laya.Matrix();
    ...
    class Templet extends AnimationTemplet {
        ...
        _parse(texture, createURL, skeletonData) {
            ...
            var reader = new Laya.Byte(skeletonData);
            this._aniVersion = reader.readUTFString();
            AnimationParser01.parse(this, reader);
    ...
    class TempletLoader {
        load(task) {
            return Promise.all([
                ...
                task.loader.load(Laya.Utils.replaceFileExtension(task.url, "png"), null, task.progress.createCallback())
            ]).then(res => {
                if (!res[0] || !res[1])
                    return null;
    ...
    Laya.Loader.registerLoader(["sk"], TempletLoader);
    ...
    class MovieClip extends Laya.Sprite {
        ...
        _clear() {
            ...
            for (key in this._loadedImage) {
                ...
                if (obj) {
                    Laya.ILaya.Loader.clearRes(key, obj);
        ...
        load(url, atlas = false, atlasPath = null) {
            ...
            Laya.ILaya.loader.load(atlasPath ? [url, atlasPath] : [url], Laya.Loader.BUFFER).then(res => {
                var _a;
                if (!res) {
                ...
                this._initData(new Laya.Byte(res[0].data), basePath);
            });
        }
        _initState() {
            this._reset();
